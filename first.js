...
let form = document.getElementById('astrologyForm');

form.addEventListener('submit',(event) =>{
    event.preventDefault();
    // M-1 : by formdata, but ye thoda complicated ho jae ga

    // M-2 : by general method
    const first = document.getElementById('name').value;
    const last = document.getElementById('surname').value ;
    const date = Number(document.getElementById('date').value) ;  // input tag will return string
    const month = Number(document.getElementById('month').value) ;  
    const year = Number(document.getElementById('year').value) ;  
    // console.log(first,last,date,year)
    // message
    const zeromssg = `Hii, ${first} ${last}`;
    const firstmssg = `Your Zodiac Sign is ${zodiacSigns[month-1]}`;
    const secondmssg = `${compliments[date-1]}`;
    let index = Math.floor(Math.random()*20);
    const thirdmssg = `${victimCardCompliments[index]}`;
    index = (first.length * last.length * month)%30;
    const forthmssg = `Recommendation for you, ${recommendations[index]}`;
    index = (first.length * last.length * year)%20;
    const fifthmssg = `${predictions[index]}`;

    let result = document.getElementById('result');
    result.innerHTML = `${zeromssg} ${firstmssg} ${secondmssg} ${thirdmssg} ${forthmssg} ${fifthmssg}`;
})

// HW: 1) issme whatshap pe mssg direct kr doo
//     2) zomato vala filter application
//     3) 10 basic projects
//     4) revision of js
